...
template<class TYP, int ROZMIAR>
const Wektor<TYP,ROZMIAR> & Wektor<TYP,ROZMIAR>::operator+=(const Wektor<TYP,ROZMIAR> &W2)
{
    for(int i=0;i<ROZMIAR;i++)
    {
        this->tab[i] = this->tab[i]+W2[i];
    }
    return *this;
}
...
template<class TYP, int ROZMIAR>
const Wektor<TYP,ROZMIAR> & Wektor<TYP,ROZMIAR>::operator*=(double li)
{
    for(int i=0;i<ROZMIAR;i++)
    {
        this->tab[i] = this->tab[i]*li;
    }
    return *this;
}
